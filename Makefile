...
runtime:
	...
	@if [ -f "../build/$(MISSION)/$(SPACECRAFT)/sim/libsimulith.so" ]; then \
		...
		cp "../build/$(MISSION)/$(SPACECRAFT)/fsw/amd64-tryspace/default_cpu1/simulith/libsimulith.so" "../build/$(MISSION)/$(SPACECRAFT)/lib/libsimulith.so"; \
	fi
